...
public class MarkerIconCommand extends FCommand {

    public MarkerIconCommand() {
        setCommand("icon");
        setMinMaxArgs(2, 2);
        setPermissionFromName(MarkerCommand.LABEL);
        setFUsage(MarkerCommand.LABEL + " " + getCommand() + " <id> <iconPath>");
        setDescription("Sets the icon path for a marker");
    }
    ...
    public void onExecute(CommandSender sender, String[] args) {
        ...
        marker.setIconPath(args[2]);
        marker.save();
        MessageUtil.sendMessage(sender, "<gray>Set icon path to: <green>" + args[2]);
    }
